...
//hey you beautiful people :D -Ethan
...
SengledHubPlatform.prototype.addAccessory = function(data) {
    if (!this.accessories[data.id]) {
        ...
        //newAccessory.addService(Service.Lightbulb, data.name)
		//it's been said that i don't know what i'm doing. -ethan
		newAccessory
...
SengledHubPlatform.prototype.getBrightness = function(thisPlug, callback) {
    if (this.accessories[thisPlug.id]) {
        this.log("Getting Brightness: %s %s", thisPlug.id, thisPlug.name)
        ...
        return this.client.login(this.username, this.password).then( () => {
            ...
            if (typeof device === 'undefined') {
                ...
                thisPlug.brightness = device.brightness;
                if (this.debug) this.log("getBrightness complete");
...
SengledHubPlatform.prototype.setBrightness = function(thisPlug, powerState, callback) {
    let that = this;
    if (this.debug) this.log("Sending device: " + thisPlug.id + " brightness change to " + powerState);
    return this.client.login(this.username, this.password).then( () => {
        return this.client.deviceSetBrightness(thisPlug.id, powerState);
    }).then( () => {
        callback();
    }).catch( (err) => {
        this.log("Failed to set brightness to", powerState);
        callback(err);
    });
};
